...
int WINAPI WinMain(_In_ HINSTANCE, _In_opt_ HINSTANCE, _In_ LPSTR, _In_ int) {
    ...
    VertexData vertices[] =
    {
        {{-1.0f,  1.0f, 0.0f, 1.0f},{ 0.0f, 0.0f}},  // 0左上
        {{ 1.0f,  1.0f, 0.0f, 1.0f},{ 1.0f, 0.0f}},  // 1右上
        {{-1.0f, -1.0f, 0.0f, 1.0f},{ 0.0f, 1.0f}},  // 2左下
        {{ 1.0f, -1.0f, 0.0f, 1.0f},{ 1.0f, 1.0f}},  // 3右下
    };
    ...
    // 0.RenderTextureResoueceの作成
    const FLOAT kRenderTargetClearColor[4] = { 1.0f,0.0f,0.0f,1.0f };  // 画面クリア色(分かりやすいように赤に）
    ...
    dsvDesc.Format = DXGI_FORMAT_D32_FLOAT;                 // 基本的にResourceに合わせた
    dsvDesc.ViewDimension = D3D12_DSV_DIMENSION_TEXTURE2D;  // 2DTexture

    // DSVHeapの先頭にDSVを作る
    device->CreateDepthStencilView(depthStencilResource, &dsvDesc, dsvHandleCPU);

    // SRV(ShaderRessourceView)を準備する -------------------- ☆00_09追加
    ...
    while (true) {
        ...
        barrier.Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;                        // TranslationBarrireの設定
...
ID3D12Resource* CreateRenderTextureResource(ID3D12Device* device, uint32_t width, uint32_t height, DXGI_FORMAT clearFormat, const FLOAT* clearColor)
{
    // 1.生成するRenderTargetのDesc設定
    D3D12_RESOURCE_DESC resourceDesc{};
    resourceDesc.Width = UINT(width);                              // RenderTargetの幅
    ...
    HRESULT hr = device->CreateCommittedResource(
        ...
        D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE,  // PixelShaderでアクセスできるように
        &clearValue,                                 // Clear最適値
...
ID3D12Resource* CreateDepthStencilTextureResource(ID3D12Device* device, int32_t width, int32_t height)
{
    ...
    resourceDesc.MipLevels = 1;                                    // mipmapの数　DehtpStencilなので１つでいい
    resourceDesc.DepthOrArraySize = 1;                             // Textureの配列数　DehtpStencilは１つでいい
    resourceDesc.Format = DXGI_FORMAT_D32_FLOAT;                   // DehtpStencilとして利用可能なフォーマット　※KamataEngineと合わせる
    resourceDesc.SampleDesc.Count = 1;                             // サンプリングカウント　１固定
    resourceDesc.Dimension = D3D12_RESOURCE_DIMENSION_TEXTURE2D;   // ２次元
    resourceDesc.Flags = D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL;  // DehtpStencilとして使う通知
    ...
    depthClearValue.Format = DXGI_FORMAT_D32_FLOAT;  // Zバッファ形式、reosurceと合わせる　※KamataEngineと合わせた
